...
class ControlFrame(Tab):
  ...
  def __init__(self, parentFrame, headlight_controls):
    global root
    
    self.parentFrame = parentFrame
    self.headlight_controls = headlight_controls
    self.tkFrame_headlight_control = tk.LabelFrame(parentFrame, text='rFactor Headlight Control', padx=self.xyPadding,  pady=self.xyPadding)

    tk.Label(self.tkFrame_headlight_control, text="Controller").grid(row = 0, column = 1)
    tk.Label(self.tkFrame_headlight_control, text="Control").grid(row = 0, column = 2)

    ##########################################################
    for _control_num, (name, control) in enumerate(self.headlight_controls.items()):
      _control_line = self.headlight_controls[name]
      _control_line['tkButton'] = tk.Button(self.tkFrame_headlight_control, text='Select ' + name, width=20, 
                                 command=lambda n=name, w=super().controller_o: self.set_control(n,w))
      _control_line['tkButton'].grid(row = _control_num+2, sticky='w', pady=3)
      control['JoystickButton'] = super().config_o.get(name, 'Control')
      ...
      _control_line['tkLabel'].grid(row = _control_num+2, column=2, sticky='w')
      _control_line['svControl'] = tk.StringVar()

      control['ControllerName'] = super().config_o.get(name, 'Controller')
      _control_line['ControllerName'] = tk.Label(self.tkFrame_headlight_control, 
                                ...
                                relief=tk.GROOVE,
                                borderwidth=4, anchor='e', padx=4)
      _control_line['ControllerName'].grid(row = _control_num+2, column=1, sticky='w')
...
class rFactorHeadlightControlFrame(ControlFrame):
    def __init__(self, parentFrame):
        ...
        ##########################################################
        self._separator = ttk.Separator(self.tkFrame_headlight_control, orient="horizontal")
